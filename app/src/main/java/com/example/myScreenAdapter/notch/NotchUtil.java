...
public class NotchUtil {
     ...
     * @param isLightMode 状态栏模式
     */
    public static void setImmersiveWidthNotch(Context context,boolean isLightMode,int cutoutMode){
     ...
     * @param isLightMode 状态栏模式
     */
    public static void setNoneImmersiveWidthNotch(Context context,boolean isLightMode,int cutoutMode){
     ...
     */
    private static void setImmersiveBarsMode(Context context,boolean isUseImmersiveBars,boolean isUseCutout,boolean isLightMode, int cutoutMode){
        if (context instanceof AppCompatActivity){
            AppCompatActivity app = (AppCompatActivity) context;
            if (isUseImmersiveBars){
                //由于actionBar可能在style总设置隐藏，那么app.getSupportActionBar()可能为空，故做空判断
     ...
     */
    public static <T extends ViewGroup.MarginLayoutParams> void adaptStatusHeight(Activity activity, View view, int originalMargin, boolean isDP){
     ...
     * @param <T>            包裹view的ViewGroup的Layoutparams
     */
    public static <T extends ViewGroup.MarginLayoutParams> void restoreMarginHeight(Activity activity,View view,int originalMargin,boolean isDP){
        T lp = (T) view.getLayoutParams();
        if (isDP){
            originalMargin = ScreenUtil.dip2px(activity,originalMargin);
        }
